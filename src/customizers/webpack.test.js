...
describe("addLessLoader", () => {
  const findLessLoaderConf = (loader) =>
    loader.test && loader.test.test("a.less");
  const findLessModuleLoaderConf = (loader) =>
    loader.test &&
    loader.test.test("a.module.less") &&
    !loader.test.test("a.less");
  ...
  test("with options", () => {
    ...
    // all loaders
    const loaders = finalConfig.module.rules.find((rule) =>
    ...
    expect(
      lessLoder.use.find((loaderConf) => isCSSLoader(loaderConf.loader)).options
    ).toEqual({
      importLoaders: 2,
    ...
    expect(
      lessLoder.use.find((loaderConf) => isLessLoader(loaderConf.loader))
        .options
    ).toEqual({
      sourceMap: shouldUseSourceMap,
      lessOptions: {
    ...
    expect(
      lessModuleLoder.use.find((loaderConf) => isCSSLoader(loaderConf.loader))
        .options
    ).toEqual({
      importLoaders: 2,
      sourceMap: shouldUseSourceMap,
    ...
    expect(
      lessModuleLoder.use.find((loaderConf) => isLessLoader(loaderConf.loader))
        .options
    ).toEqual({
      sourceMap: shouldUseSourceMap,
      lessOptions: {
